...
# --- private-manifest discovery (the apollo-client shape) ---------------------


def test_private_manifest_names_become_candidates():
    from scanner.ecosystems import private_manifest_candidates
    texts = {
        "package.json": '{"name": "@apollo/client", "private": true}',
        "codegen/package.json": '{"name": "@apollo/client-graphql-codegen"}',
        "web/package.json": '{"private": true}',   # nameless: nothing to ask about
        "Cargo.toml": '[package]\nname = "irrelevant"\n',
    }
    assert private_manifest_candidates(texts) == [("npm", "@apollo/client")]


def test_collect_ecosystem_recovers_a_private_flagship_the_registry_confirms(monkeypatch):
    """apollographql/apollo-client's shape: the workspace root declares the
    flagship name but marks it private (release tooling strips the flag), and
    a satellite package resolves fine. The flagship must still be asked about
    — and kept, because npm's entry points back at this repository."""
    import scanner.ecosystems as eco
    ...
    def fake_npm(client, name, repo_full_name, contacts=None):
        ...
        return EcosystemPackage(
            ecosystem="npm", name=name, registry_url="https://x",
            monthly_downloads=monthly, matches_repo=True,
        )

    monkeypatch.setitem(eco.FETCHERS, "npm", fake_npm)
    packages, _, _, _ = collect_ecosystem(
        "apollographql", "apollo-client",
        "main", ["package.json", "codegen/package.json"], [],
    )

    by_name = {p.name: p for p in packages}
    assert "@apollo/client" in by_name
    assert by_name["@apollo/client"].monthly_downloads == 25_000_000
    assert "@apollo/client-graphql-codegen" in by_name


def test_a_genuinely_private_name_the_registry_disowns_stays_out(monkeypatch):
    """A private manifest whose name belongs to a stranger on npm must not
    appear on the report — same verification gate as filename guesses."""
    import scanner.ecosystems as eco
    ...
    def fake_npm(client, name, repo_full_name, contacts=None):
        return EcosystemPackage(
            ecosystem="npm", name=name, registry_url="https://x",
            matches_repo=(name == "acme-lib") or None if name == "acme-lib" else None,
        )

    monkeypatch.setitem(eco.FETCHERS, "npm", fake_npm)
    packages, _, _, _ = collect_ecosystem(
        "acme", "app", "main", ["package.json", "lib/package.json"], [],
    )
    assert "internal-app" not in {p.name for p in packages}
