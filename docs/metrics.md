...
| Machine-readable docs (llms.txt) | 15 | `llms.txt` / `llms-full.txt` in the repository tree, or served by the project's website (declared homepage root, or a documentation site the README links to) — docs toolchains build the file at docs-build time without committing it |
